...
import { DocumentConstants } from '../../../candidate/constants/DocumentConstants';
...
export class SkillComponent implements OnChanges {
  ...
  showSkillDeleteConfirm(skillId: string): void {
    this.modal.confirm({
      ...
      nzOkDanger: true,
      nzOnOk: () => this.skillService.deleteSkill(skillId).subscribe({
        next:
          () => {
            this.skills = this.skills.filter((skill) => skill.userSkillGuid !== skillId)
            this.message.success(sharedConstant.SKILL_DELETE)
          },
        error: () => this.message.error(sharedConstant.SKILL_ERROR_DELETE)
      }),
