...
export class TrainingComponent implements OnChanges {
  ...
  constructor(
    ...
    this.formTraining = this.fb.group({
      ...
      userTrainingDescription: ['', [maxLength(500), pattern(ManagementValidationConstants.REGEX_PATTERN_NO_ANGELBRACKETS)]],
    });
  }
        ...
        this.trainingService
          ...
          .subscribe({
            ...
            error: () => {
              this.formTraining.reset();
              this.isTrainingModalVisible = false;
              this.msg.error(sharedConstant.TRAINING_ERROR_ADD)
            }
          });
      } else {
        this.trainingService
          ...
          .subscribe({
            ...
            error: () => {
              this.formTraining.reset();
              this.isTrainingModalVisible = false;
              this.msg.error(sharedConstant.TRAINING_ERROR_UPDATE)
            }
          });
      }
    } else {
  ...
  showTrainingDeleteConfirm(trainingId: string): void {
    this.modal.confirm({
      nzTitle: ManagementDocumentConstants.TRAINING_DELETE_CONFIRMATION_MSG,
      nzCentered: true,
