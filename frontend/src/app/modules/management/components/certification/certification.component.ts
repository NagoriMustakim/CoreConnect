...
export class CertificationComponent implements OnChanges {
  ...
  constructor(
    ...
    this.formCertificate = this.fb.group({
      ...
      certificationCredentialId: ['', [maxLength(250), pattern(ManagementValidationConstants.REGEX_PATTERN_ALPHANUMERICS)]],
      certificationCredentialUrl: ['', [maxLength(500), pattern(ManagementValidationConstants.REGEX_PATTERN_NO_BRACKETS)]],
      certificationPhotoName: ['', []],
      certificationDescription: ['', [maxLength(500), pattern(ManagementValidationConstants.REGEX_PATTERN_NO_ANGELBRACKETS)]],
    });
  }
  ...
  fallback = ManagementDocumentConstants.FALLBACK_IMAGE;
  ...
  showCertificationDeleteConfirm(certificationId: string): void {
    this.modal.confirm({
      nzTitle: ManagementDocumentConstants.CERTIFICATIONS_DELETE_CONFIRMATION_MSG,
      nzCentered: true,
      ...
      nzOnOk: () =>
        this.certificationService
          ...
          .subscribe({
            ...
            error: () => this.message.error(sharedConstant.CERTIFICATE_ERROR_DELETE)
          }
          ),
