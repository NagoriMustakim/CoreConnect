...
export class ExperienceComponent implements OnChanges {
  ...
  constructor(
    ...
    this.formExperience = this.fb.group({
      ...
      experienceDescription: ['', [maxLength(500), pattern(ManagementValidationConstants.REGEX_PATTERN_NO_ANGELBRACKETS)]],
      employmentTypeName: [''],
    });
  }
  ...
  showExperienceModal(experience?: Experience): void {
    if (experience) {
        ...
        this.experienceService
          ...
          .subscribe({
            ...
            error: () => {
              this.formExperience.reset();
              this.isExperienceModalVisible = false;
              this.msg.error(sharedConstant.EXPERIENCE_ERROR_ADD)
            }
          });
      } else {
        ...
        this.experienceService
          ...
          .subscribe({
            ...
            error: () => {
              this.formExperience.reset();
              this.isExperienceModalVisible = false;
              this.msg.error(sharedConstant.EXPERIENCE_ERROR_UPDATE)
            }
          });
      }
    } else {
  ...
  showExperienceDeleteConfirm(experienceId: string): void {
    this.modal.confirm({
      nzTitle: ManagementDocumentConstants.EXPERIENCE_DELETE_CONFIRMATION_MSG,
      nzCentered: true,
  ...
  getStatesByCountry(value: string) {
    if (value) {
      this.formExperience.controls.state.reset();
      this.formExperience.controls.city.reset();
