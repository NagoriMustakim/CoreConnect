...
export class ProjectComponent implements OnChanges {
  ...
  constructor(
    ...
    this.formProject = this.fb.group({
      ...
      projectDescription: [
        ...
        [
          maxLength(500),
          pattern(ManagementValidationConstants.REGEX_PATTERN_NO_ANGELBRACKETS),
        ],
      ],
      skillsGuids: [''],
    });
      ...
      if (this.formProject.value.userProjectGuid === '') {
        this.projectService.addProject(this.formProject.value).subscribe({
          next:
            (response: IProject) => {
              ...
              this.msg.success(sharedConstant.PROJECT_ADD)
              this.startValue = null;
              this.endValue = null;
            },
          error: () => {
            this.msg.error(sharedConstant.PROJECT_ERROR_ADD)
          }
        });
      } else {
        this.projectService.updateProject(this.formProject.value).subscribe({
          next:
            () => {
              this.getData();
              let index = this.projects.findIndex((project) => this.formProject.value.userProjectGuid == project.userProjectGuid);
              this.projects[index] = { ...this.projects[index], ...this.formProject.value };
              this.listOfSelectedSkills=[];
              this.formProject.reset();
              this.isProjectModalVisible = false;
              this.msg.success(sharedConstant.PROJECT_UPDATE)
              this.startValue = null;
              this.endValue = null;
            },
          error: () => {
            this.msg.success(sharedConstant.PROJECT_ERROR_UPDATE)
          }
        });
      }
    } else {
  ...
  showProjectDeleteConfirm(projectId: string): void {
    this.modal.confirm({
      nzTitle: ManagementDocumentConstants.PROJECT_DELETE_CONFIRMATION_MSG,
      nzCentered: true,
      ...
      nzOkDanger: true,
      nzOnOk: () => this.projectService.deleteProject(projectId).subscribe({
        next:
          () => {
            this.projects = this.projects.filter((project) => project.userProjectGuid !== projectId)
            this.msg.success(sharedConstant.PROJECT_DELETE)
          },
        error: () => this.msg.error(sharedConstant.PROJECT_ERROR_DELETE)
      }),
