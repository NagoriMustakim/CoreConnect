...
export class TrainingComponent implements OnInit {
  ...
  constructor(
    ...
    this.formTraining = this.fb.group({
      ...
      userTrainingDescription: ['', [maxLength(500), pattern(ValidationConstants.REGEX_PATTERN_NO_ANGELBRACKETS)]],
    });
  }
    ...
    if (this.formTraining.valid) {
      if (this.formTraining.value.userTrainingGuid === '') {
        this.trainingService
          .addTraining(this.formTraining.value)
          .subscribe({
            next:
              (response: ITraining) => {
                this.gettrainings()
                ...
                this.message.success(sharedConstant.TRAINING_ADD)
                this.startValue = null;
                this.endValue = null;
              },
            error: () => this.message.error(sharedConstant.TRAINING_ERROR_ADD)
          });
      } else {
        this.trainingService
          .updateTraining(this.formTraining.value)
          .subscribe({
            next:
              (response: ITraining) => {
                ...
                this.formTraining.reset();
                this.isTrainingModalVisible = false;
                this.message.success(sharedConstant.TRAINING_UPDATE)
                this.startValue = null;
                this.endValue = null;
              },
            error: () => this.message.error(sharedConstant.TRAINING_ERROR_UPDATE)
  ...
  showTrainingDeleteConfirm(trainingId: string): void {
    this.modal.confirm({
      nzTitle: DocumentConstants.TRAINING_DELETE_CONFIRMATION_MSG,
      nzCentered: true,
          ...
          .subscribe({
            next:
              () => {
                ...
                this.message.success(sharedConstant.TRAINING_DELETE)
              },
            error: () => this.message.error(sharedConstant.TRAINING_ERROR_DELETE)
          }),
  ...
  handleStartOpenChange(): void {
    if (this.formTraining.controls.trainingEndDate.value && this.formTraining.controls.trainingEndDate.value !== '') {
      this.startValue = new Date(this.formTraining.controls.trainingEndDate.value);
    }
    else
      this.startValue = null;
  }
  ...
  handleEndOpenChange(): void {
    if (this.formTraining.controls.trainingEndDate.value && this.formTraining.controls.trainingEndDate.value !== '') {
      this.endValue = new Date(this.formTraining.controls.trainingEndDate.value);
    }
    else
      this.endValue = null;
  }
