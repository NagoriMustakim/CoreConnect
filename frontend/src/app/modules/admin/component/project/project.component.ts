...
import { IProject } from '../../../candidate/interfaces/IProject';
  ...
  handleProjectModalOk(): void {
    if (this.formProject.valid) {
      ...
      if (this.formProject.value.userProjectGuid === '') {
        this.projectService
          .addProject(this.userId, this.formProject.value)
          .subscribe({
            next: (response: any) => {
              this.getData();
            ...
            error: () => this.msg.error(sharedConstant.PROJECT_ERROR_ADD),
          });
      } else {
        this.projectService
          .updateProject(this.userId, this.formProject.value)
          .subscribe({
            next: () => {
              this.getData();
            ...
            error: () => this.msg.error(sharedConstant.PROJECT_ERROR_UPDATE),
          });
      }
    } else {
