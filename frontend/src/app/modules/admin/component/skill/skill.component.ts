...
export class SkillComponent {
  ...
  handleSkillModalOk(): void {
    if (this.formSkill.valid) {
      if (this.formSkill.value.userSkillGuid === '') {
        this.skillService.AddUserSkill(this.formSkill.value, this.userId).subscribe({
          next:
            (response: any) => {
        ...
        this.skillService.UpdateUserSkill(this.formSkill.value, this.userId).subscribe({
          next:
            (response: any) => {
              this.getData()
              let index = this.skills.findIndex((skill) => this.formSkill.value.userSkillGuid == skill.userSkillGuid);
              this.skills[index] = { ...this.skills[index], ...this.formSkill.value };
  ...
  showSkillDeleteConfirm(skillId: string): void {
    this.modal.confirm({
      ...
      nzOkDanger: true,
      nzOnOk: () => this.skillService.DeleteUserSkill(skillId, this.userId).subscribe(
        {
          next:
            () => {
              this.skills = this.skills.filter((skill) => skill.userSkillGuid !== skillId)
